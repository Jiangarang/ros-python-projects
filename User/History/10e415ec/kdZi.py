...
def cb(msg):
    global state
    global last_distance_front = 0
    global last_distance_right = 0
    global last_distance_left = 0
    global last_distance_rear = 0
    ...
    if (state == 0 and distance_front <= 1): # if wall is <= 1m ahead, begin rotating robot until shortest distance is aligned to right side
        state = 1
    elif (state == 1 and distance_): # if wall is parallel to right, start going forward
        state = 0
