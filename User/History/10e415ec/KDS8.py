...
def cb(msg):
    ...
    if (distance_front <= 1 or distance_right > last_distance_right): # if wall is <= 1m ahead, begin rotating robot until shortest distance is aligned to right side
        ...
        state = 0
    elif (distance_right > last_distance_right or distance_right < 1): # if robot is straying left, steer slightly right
        state = 2
    elif (distance_right < last_distance_right or distance_left < 1): # if robot is straying right, steer slightly left
        state = 3
    else:
        state = 0
