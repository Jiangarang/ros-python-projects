...
def cb(msg):
    ...
    #takes an average of three datapoints per direction
    ...
    if (distance_front <= 1): # if wall is <= 1m ahead, begin rotating robot until shortest distance is aligned to right side
        state = 1
    elif (state == 1 and (distance_front - distance_front < 0.3)): #robot is parallel to right, finish turning and go straight
        state = 0
    elif (state == 0 and distance_right - last_distance_right > 0): # if robot is straying left, steer slightly right
        state = 2
    elif (state == 0 and distance_right - last_distance_right < 0): # if robot is straying right, steer slightly left
        state = 3
    else:
        state = 0

    #update to keep track of last position
