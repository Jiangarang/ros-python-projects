...
def cb(msg):
    ...
    if (distance_front <= 1): # if wall is <= 1m ahead, begin rotating robot until shortest distance is aligned to right side
        state = 1
    elif (state == 1 and (distance_right - last_distance_right < 0.1)): #robot is parallel to right
        state = 0
    elif (distance_right > last_distance_right + 0.1): # if robot is straying left, steer slightly right
        state = 2
    elif (distance_right < last_distance_right - 0.1): # if robot is straying left, steer slightly right
        state = 3
    else:
