...
# default state will be set to 0 (go straight)
state = 0
...
while not rospy.is_shutdown():
    ...
    # state 0 = searching for wall, drive forward until wall is detected 1m in front
    if (state == 0):
        t_pub.linear.x = 0.2
        t_pub.angular.z = 0
    ...
    # state 2 = straying left, steer right slightly
    elif (state == 1): 
        t_pub.linear.x = 0.2
        t_pub.angular.z = 0.05

    # state 2 = straying right, steer left slightly
    elif (state == 1): 
        t_pub.linear.x = 0.2
        t_pub.angular.z = -0.05
