...
#make publisher for current state
pub_state('current_state', Int16, queue_size = 1)
...
while not rospy.is_shutdown():
    print("STATE: ", state)
    if (state == 0): # state 0 = searching for wall, slowly drive forward until wall is detected 1m away
        t_pub.linear.x = 0.2
        t_pub.angular.z = 0
    elif (state == 1): # state 1 = wall is ~1m away, rotate the robot until its right side is parallel with wall
        t_pub.linear.x = 0
        t_pub.angular.z = 0.2
    elif (state == 9): # halt
        t_pub.linear.x = 0
